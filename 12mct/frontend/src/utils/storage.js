...
export const importMeals = (profileId, meals) => {
  const existing = getMeals(profileId);
  const existingIds = new Set(existing.map((meal) => meal.id));
  const incoming = meals.filter((meal) => !existingIds.has(meal.id));
